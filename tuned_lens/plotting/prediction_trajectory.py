...
class PredictionTrajectory:
    ...
    def from_lens_and_cache(
        ...
        for layer in range(cache.model.cfg.n_layers):
            ...
            traj_log_probs.append(logits.log_softmax(dim=-1).detach().cpu().to(dtype=th.float32).numpy())

        model_log_probs = model_logits.log_softmax(-1).detach().cpu().to(dtype=th.float32).numpy()
        ...
        return cls(
            ...
            input_ids=input_ids.cpu().to(dtype=th.float32).numpy(),
            targets=None if targets is None else targets.cpu().to(dtype=th.float32).numpy(),
            anti_targets=None if anti_targets is None else anti_targets.cpu().to(dtype=th.float32).numpy(),
        )
    ...
    def from_lens_and_model(
        ...
        model_log_probs = (
            outputs.logits[..., :].log_softmax(-1).squeeze().detach().cpu().to(dtype=th.float32).numpy()
        )
        ...
        for i, h in enumerate(stream[:-1]):
            ...
            traj_log_probs.append(
                logits.log_softmax(dim=-1).squeeze().detach().cpu().to(dtype=th.float32).numpy()
            )
    ...
    def kl_divergence(
        ...
        return TrajectoryStatistic(
            ...
            units="nats",
            stats=kl_div,
            trajectory_labels=self._largest_prob_labels(**kwargs),#self._largest_delta_in_prob_labels(other, **kwargs),
            sequence_labels=self._get_sequence_labels(),
            min=0,
            max=None,
        )
